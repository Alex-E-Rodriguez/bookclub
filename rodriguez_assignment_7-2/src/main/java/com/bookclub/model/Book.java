...
public class Book {
    ...
    private String infoURL; // The URL for information on the book.
    private int numOfPages; // Book Page Count. Default zero.

    /*
     * Default constructor for the Book class.
     * @return returns an instance of Book with no isbn, title, description, info URL, or page count.
     */
    public Book() {
        this.isbn = "";
        this.title = "";
     ...
     * @return returns an instance of Book with a user-specified isbn, title, description, info URL, and page count.
     */
    ...
    {
        return String.format("Book{isbn=%1$s, title=%2$s, description=%3$s, infoURL=%4$s, numOfPages=%5$s}", isbn, title, description, infoURL, numOfPages);
    } // end toString
